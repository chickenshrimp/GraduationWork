...
class MorphableModel:
    ...
    def __init__(self, path, textureResolution = 256, trimPca = False, landmarksPathName = 'landmark_62_mp.txt', device='cpu'):
        '''
        ...
        self.landmarksAssociation = torch.tensor(np.loadtxt(pathLandmarks, delimiter=None)[:, 1].astype(np.int64)).to(device)
        self.landmarksMask = torch.tensor(np.loadtxt(pathLandmarks, delimiter=None)[:, 0].astype(np.int64)).to(device)
